...
int main() {
	...
	const std::string streamFileName = "G:/StreamsDissertation/RealesSetup/Geometrie/IdealesSpektrum_Alu_aKegel3_Sweep" + std::to_string(sweep) + "ns_" + std::to_string(SourcePower) + "uCi.drs4DataStream";


	fstream myfile("G:/StreamsDissertation/RealesSetup/Geometrie/IdealesSpektrum_Alu_aKegel3_AnalyseParameter" + std::to_string(SourcePower) + "uCi.txt", ios::out);


	//Input Streams

	ifstream ifile("C:/Users/Simulationsrechner/Desktop/Geant4MeetsPulsGenerator/Streams/1275keV_aKegel3_Al_probe_1.5mm_Abstand_3mm.stream", ios::binary);
	ifstream ifile511("C:/Users/Simulationsrechner/Desktop/Geant4MeetsPulsGenerator/Streams/511keV_aKegel3_Al_probe_1.5mm_Abstand_3mm.stream", ios::binary);
	...
	for (int i = 0; i < 50; i++) {
		...
		if (ifile.is_open() || ifile511.is_open())
		{
			...
			while (!ifile.eof()) {
				...
				double PALS = positronAnhillationLifetime(lifetime1, lifetime2, lifetime3, intensity1, intensity2, intensity3, distribution1, distribution2, distribution3, generator);
